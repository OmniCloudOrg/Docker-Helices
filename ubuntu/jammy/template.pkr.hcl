...
locals {
  ...
  # Base autoinstall configuration 
  user_data = <<EOF
#cloud-config
autoinstall:
  version: 1
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      # Optional: Add a placeholder file to verify shared folder access
      "echo 'Shared folder access test' | sudo tee /mnt/shared/test.txt",
